            ...
            <TableHead>
              <TableRow key="headerRow">
                  {props.keyValues.map((keyValue: DBtype) => (
                      <StyledTableCell key={`headerRow_${keyValue}`}>{keyValue}</StyledTableCell>
                  ))}
              </TableRow>
            </TableHead>
            <TableBody>
              {props.posts.map((post: DBentry) => {
                ...
                <TableRow key={id}>
                  {props.keyValues.map((keyValue: DBtype ) => (
                    <TableCell key={`${id}_${keyValue}`} className={classes.tableCell} component="th" scope="row">{post[keyValue]}</TableCell>
                  ))}
              </TableRow>)
              })}
            </TableBody>
