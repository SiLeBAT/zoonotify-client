...
const PrevalenceMainContent: React.FC<PrevalenceMainContentProps> = ({ heading }) => {
    ...
    useEffect(() => {
      ...
      const fetchData = async (microorganismName: string): Promise<PrevalenceDataItem[]> => {
        let allData: PrevalenceDataItem[] = []; // Define the type of allData
        let pageSize = 100; // or the maximum allowed by your API
        let page = 0;
  
        try {
          // Keep fetching data until all pages have been fetched
          while (true) {
            const response = await axios.get(`http://localhost:1337/api/prevalences`, {
              params: {
                populate: '*',
                'filters[microorganism][name][$eq]': microorganismName,
    ...
    return (
      ...
      <Paper sx={customPaperStyle}>
          ...
          <TableRow>
              ...
              <TableCell align="center">Number of Positive</TableCell>
              <TableCell align="center">Percentage of Positive</TableCell>
          ...
          <TableBody>
            {loading ? (
              <TableRow>
                <TableCell align="center" colSpan={9}>Loading...</TableCell>
              </TableRow>
            ) : error ? (
              <TableRow>
                <TableCell align="center" colSpan={9}>Error: {error}</TableCell>
              </TableRow>
            ) : (
              data.map((item) => (
                <TableRow key={item.id}>
                  <TableCell align="center">{item.attributes.samplingYear}</TableCell>
                  <TableCell align="center">{item.attributes.matrix?.attributes.name || 'N/A'}</TableCell>
                  <TableCell align="center">{item.attributes.microorganism?.attributes.name || 'N/A'}</TableCell>
                  <TableCell align="center">{item.attributes.numberOfSamples}</TableCell>
                  <TableCell align="center">{item.attributes.numberOfPositive}</TableCell>
                  <TableCell align="center">{`${item.attributes.percentageOfPositive.toFixed(2)}%`}</TableCell>
                  <TableCell align="center">{item.attributes.ciMin?.toFixed(2) || 'N/A'}</TableCell>
                  <TableCell align="center">{item.attributes.ciMax?.toFixed(2) || 'N/A'}</TableCell>
                </TableRow>
              ))
            )}
         </TableBody>
